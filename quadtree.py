...
class BoundingBox:
    ...
    def radius_meters(self):
        """Haversine distance from center to corner — guarantees full coverage."""
    ...
    def width_meters(self):
        """Approximate width of the cell — used to check minimum cell size."""
        mid_lat = (self.lat_south + self.lat_north) / 2
        return haversine(mid_lat, self.lon_west, mid_lat, self.lon_east)
...
def build_quadtree(bounding_box, depth=0):
    """
    Recursively build a quadtree over the bounding box.
    Splits a cell if the API returns MAX_RESULTS (meaning it's saturated).
    Stops splitting if the cell is smaller than MIN_CELL_METERS.
    """
    ...
    # Stopping conditions:
    # 1. Fewer than MAX_RESULTS — cell is not saturated, we're done here
    if len(results) < 20:
        node.results = results
        return node

    # Cell is saturated — split into 4 quadrants and recurse
    print(f"{indent}  Saturated — splitting into 4 quadrants")
    for child_bounding_box in bounding_box.split():
